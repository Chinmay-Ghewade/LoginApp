...
<%
    ...
    if ("search".equals(action) || "lookup".equals(action)) {
        ...
        if (term == null) term = "";
        term = term.trim();
        // For lookup with empty term — return first 50 records
    ...
    if ("get".equals(action)) {
        ...
        try {
            ...
            ps  = con.prepareStatement(
                    ...
                    "FROM BALANCE.ACCOUNT " +
                    "WHERE ACCOUNT_CODE = ?");
            // ?1=GC, ?2=GN, ?3=ANAME, ?4=CI, ?5=WHERE
...
<head>
    ...
    <style>
        ...
        *, *::before, *::after {
        ...
        input:focus {
            border-color: #5050b8;
            box-shadow: 0 0 0 2px rgba(80, 80, 184, .10);
            background: #f8f8ff;
        }
        ...
        .spin {
            ...
            width: 14px;
            height: 14px;
            border: 2px solid #d0d0ee;
            border-top-color: #3d4db7;
            border-radius: 50%;
        ...
        .lk-head {
            ...
            gap: 12px;
            padding: 14px 18px;
            border-bottom: 1.5px solid #e8e8f4;
            flex-shrink: 0;
        }
...
<body>
    ...
    <div class="box">
                    ...
                    <div class="fg">
                        ...
                        <div class="ib">
                            ...
                            <button class="btn-add" type="button"
                                    onclick="doAddPayment()">Add</button>
                        </div>
                    </div>
                </div>
            ...
            <div class="module">
                ...
                <div class="fg-row3">
                    <div class="fg">
                        ...
                        <input type="number" id="noShares" placeholder="0" oninput="calcAmt()"/>
                    </div>
                    <div class="fg">
                        <label>Face Value</label>
                        <input type="number" id="faceVal" value="1" oninput="calcAmt()"/>
                    </div>
                    <div class="fg">
                        <label>Amount</label>
        ...
        <!-- ── ACCOUNT DETAILS — 4-col grid matching Image 1 ── -->
        <div id="acDetails">
    ...
    <!-- ── Status Message Bar ── -->
    <div class="msg-bar" style="margin-bottom:10px;">
        <input id="msgBox" type="text" readonly placeholder="Status…" />
    </div>
    ...
    <script>
        ...
        function onAcInput(v) {
            document.getElementById('accountCode').value = v;
            if (v !== _prev) { clearAcDetails(); _prev = v; }
            liveSearch(v, 'dropMain', 'main');
        }

        function onTrInput(v) {
            document.getElementById('trCode').value = v;
        ...
        function doSearch(term, dropId, target) {
            ...
            xhr.open('GET', PAGE_URL + '?action=search&term=' + encodeURIComponent(term), true);
            xhr.onreadystatechange = function () {
                if (xhr.readyState !== 4) return;
                console.log('SEARCH status=' + xhr.status + ' len=' + xhr.responseText.length + ' raw=' + xhr.responseText.substring(0, 120));
                if (xhr.status !== 200) {
                    drop.innerHTML = '<div class="sr-hint">HTTP Error ' + xhr.status + '</div>';
                    return;
                }
                var raw = xhr.responseText;
                var si  = raw.indexOf('{');
                if (si > 0) raw = raw.substring(si);
                raw = raw.trim();
                var d;
                try { d = JSON.parse(raw); }
                catch (e) {
                    drop.innerHTML = '<div class="sr-hint">ERR: ' + xe(raw.substring(0, 100)) + '</div>';
                    return;
                }
        ...
        function fetchAc(code) {
            ...
            xhr.onreadystatechange = function () {
                if (xhr.readyState !== 4) return;
                console.log("XHR status:", xhr.status, "| response:", xhr.responseText.substring(0,300));
                document.getElementById('spinMain').style.display = 'none';
                if (xhr.status !== 200) {
                    clearAcDetails();
                    setMsg('Server error: ' + xhr.status, true);
                    return;
                }
                var d;
                try { var raw = xhr.responseText; var si = raw.indexOf("{"); if(si > 0) raw = raw.substring(si); d = JSON.parse(raw.trim()); }
                catch (e) { clearAcDetails(); setMsg('Parse error: ' + e.message + ' | RAW: ' + xhr.responseText.substring(0,120), true); return; }
                if (d && d.ok === true) {
                    sv('accountName',    d.n    || '');
                    sv('glCode',         d.gc   || '');
                    sv('custId',         d.ci   || '');
        ...
        function calcAmt() {
            var s = parseFloat(document.getElementById('noShares').value) || 0;
            var f = parseFloat(document.getElementById('faceVal').value)  || 0;
        ...
        function setMsg(txt, isErr) {
            // safe — msgBox may or may not exist in DOM
            var b = document.getElementById('msgBox');
            if (!b) { console.log((isErr ? "ERR: " : "OK: ") + txt); return; }
            b.value              = txt;
        ...
        function doCancel() {
            ...
            clearAcDetails();
            ['accountCode','trCode','trName','payAmt','trParticular','noShares','meetDate'].forEach(function (id) {
                var el = document.getElementById(id);
                if (el) el.value = '';
            });
            sv('faceVal',   '1');
            sv('txnAmt',    '0.00');
            sv('particular','By Cash');
            document.getElementById('dropMain').classList.remove('on');
            document.getElementById('dropTr').classList.remove('on');
        ...
        document.addEventListener('DOMContentLoaded', function() {
            document.addEventListener('click', function (e) {
                if (e.target && e.target.closest && !e.target.closest('.sw')) {
                    document.getElementById('dropMain').classList.remove('on');
                    document.getElementById('dropTr').classList.remove('on');
                }
            });
        ...
        function lkClose() {
            document.getElementById('lkOverlay').classList.remove('open');
        }
        ...
        function lkLoad(term) {
            ...
            var xhr = new XMLHttpRequest();
            var lkUrl = PAGE_URL + '?action=search&term=' + encodeURIComponent(term);
            console.log('lkLoad URL:', lkUrl);
            xhr.open('GET', lkUrl, true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState !== 4) return;
                console.log('lkLoad status=' + xhr.status + ' raw=' + xhr.responseText.substring(0,150));
                var d;
                try {
                    var raw = xhr.responseText;
                    ...
                    raw = raw.trim();
                    if (!raw) {
                        tbody.innerHTML = '<tr><td colspan="3" class="lk-err">Empty response. URL: ' + xe(lkUrl) + '</td></tr>';
                        return;
                    }
                    d = JSON.parse(raw);
                } catch(e) {
                    tbody.innerHTML = '<tr><td colspan="3" class="lk-err">ERR: ' + xe(xhr.responseText.substring(0,120)) + '</td></tr>';
                    return;
                }
    ...
    <div class="lk-overlay" id="lkOverlay" onclick="if(event.target===this)lkClose()">
        <div class="lk-modal">
            <div class="lk-head">
                ...
                <span class="lk-head-badge" id="lkBadge">ACCOUNT</span>
