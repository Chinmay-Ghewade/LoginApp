...
<%
    ...
    String subscrollNumber = request.getParameter("subscrollNumber");
    ...
    try {
        ...
        psTransaction = conn.prepareStatement(
            ...
            "WHERE SCROLL_NUMBER = ? " +
            "  AND SUBSCROLL_NUMBER = ? " +
            "  AND TRANSACTIONSTATUS = 'E' " +
            "  AND TRANSACTIONINDICATOR_CODE LIKE 'TR%' "
        );
        psTransaction.setString(1, scrollNumber);
        psTransaction.setString(2, subscrollNumber != null ? subscrollNumber : "");
        rsTransaction = psTransaction.executeQuery();

        if (!rsTransaction.next()) {
            out.println("<h3 style='color:red;'>No transaction found with number: " + scrollNumber + " / " + subscrollNumber + "</h3>");
            return;
        }
%>
...
<head>
  <meta charset="UTF-8">
  <title>View Transfer Transaction — <%= scrollNumber %></title>
  ...
  <script>
	  window.onload = function() {
		    ...
		    if (window.parent && window.parent.updateParentBreadcrumb) {
		        window.parent.updateParentBreadcrumb(
		            window.buildBreadcrumbPath('viewTransactionDetailsTransfer.jsp', 'authorizationPendingTransactionTransfer.jsp')
		        );
		    }
		};
    ...
    function goBackToList() {
        ...
        window.location.href = 'authorizationPendingTransactionTransfer.jsp';
    }
    ...
    window.onclick = function(event) {
        ...
        if (event.target === balanceErrorModal)    closeBalanceErrorModal();
    }
    ...
    function showTransactionVoucher() {
        // ✅ Get scroll number and subscroll number from URL parameters
        const urlParams = new URLSearchParams(window.location.search);
        const voucherScrollNumber = urlParams.get('scrollNumber');
        const voucherSubscrollNumber = urlParams.get('subscrollNumber');
        ...
        fetchVoucherData(voucherScrollNumber, voucherSubscrollNumber, sessionWorkingDate);
    ...
    function fetchVoucherData(scrollNumber, subscrollNumber, workingDate) {
        ...
        const params = new URLSearchParams({
            ...
            subscrollNumber: subscrollNumber,
...
<form>
    ...
    <fieldset>
      ...
      <div class="form-grid">
        ...
        <div>
          <label>TRANIDENTIFICATION ID</label>
...
<div id="authorizeModal" class="confirmation-modal">
    <div class="confirmation-modal-content">
        ...
        <p>Are you sure you want to <strong>authorize</strong> this transaction?<br>Scroll Number: <strong><%= getStringSafe(rsTransaction, "SCROLL_NUMBER") %></strong><br>Sub-Scroll Number: <strong><%= getStringSafe(rsTransaction, "SUBSCROLL_NUMBER") %></strong></p>
...
<div id="rejectModal" class="confirmation-modal">
    <div class="confirmation-modal-content">
        ...
        <p>Are you sure you want to <strong>reject</strong> this transaction?<br>Scroll Number: <strong><%= getStringSafe(rsTransaction, "SCROLL_NUMBER") %></strong><br>Sub-Scroll Number: <strong><%= getStringSafe(rsTransaction, "SUBSCROLL_NUMBER") %></strong></p>
        <div class="confirmation-modal-buttons">
